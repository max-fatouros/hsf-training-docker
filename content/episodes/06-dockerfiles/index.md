...
questions = ['How are Dockerfiles written?', 'How are images built?']
teaching = 30
...
{{< callout type="note" title="Dockerfile layers (or: why all these '&&'s??)" >}}
...
{{< callout type="note" title="Garbage cleanup" >}}
...
{{< callout type="note" title="Don't run as `root`" >}}
...
{{< challenge title="Add your own tag" >}}
...
{{< callout type="note" title="Tags are labels" >}}
...
# Dockerfile.add
FROM almalinux:9
ADD https://raw.githubusercontent.com/hsf-training/hsf-training-docker/refs/heads/gh-pages/examples/main.c .
RUN dnf -y update && \
    dnf -y upgrade && \
    dnf -y install clang && \
    dnf clean all && \
...
{{< callout type="note" title="Build compatibility" >}}
...
[c-file]: https://raw.githubusercontent.com/hsf-training/hsf-training-docker/refs/heads/gh-pages/examples/main.c
